...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        if len(remote_buffer):
            print "Receive %d bytes from remotehost" %len(remote_buffer)
            hexdump(remote_buffer)
        ...
        if not len(local_buffer) or len(remote_buffer):
            client_socket.close()
            remote_socket.close()
            print "[END] No more data Closing Connetion!"
            break
...
def main():
    ...
    if len(sys.argv[1:]) != 5:
        print "USAGE: ./proxy.py [localhost] [local port] [remote host] [remote port] [receive first]"
    ...
    #receive or not before sending remote host
